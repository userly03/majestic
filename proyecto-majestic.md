...
**Fase 1 — Reconocimiento.** Lee lineage, schemas, ownership, freshness vía el SDK Python de DataHub (`DataHubGraph`). El core (`MajesticAgent`/`ImpactSimulator`) no depende de un solo frontend: `main.py` es la CLI, y `src/mcp_server.py` expone `diagnose`/`impact` como herramientas MCP (`majestic_diagnose`, `majestic_impact`) para que otros agentes las invoquen directamente — mismo core, dos superficies. Calcula métricas estructurales (profundidad, upstream/downstream count).

**Fase 2 — Diagnóstico.** Ante una anomalía, recorre el lineage hacia atrás buscando evidencia real en el grafo (no especulación del LLM). Máximo 3 eslabones causales, cada uno respaldado por un dato concreto: tag de incidente, ausencia de owner, dataset obsoleto (freshness) o schema modificado recientemente — cuatro tipos de evidencia implementados hoy en `src/core/diagnoser.py` (no incluye assertions de DataHub todavía). Si no hay evidencia, la cadena se detiene ahí. Un mecanismo adicional ("lag-aware", ver `docs/LAG_AWARE_DIAGNOSIS.md`) ajusta el peso de cada evidencia por antigüedad y por herencia upstream, y rankea hasta 3 candidatos en vez de devolver uno solo.

**Fase 3 — Memoria.** Guarda el diagnóstico como `structuredProperties` sobre la entidad: una firma de patrón determinista (`tipo_anomalía:profundidad:upstream:downstream:plataforma`), el diagnóstico en texto, un campo de **contexto de negocio / lección aprendida** (texto libre que un humano puede enriquecer al cerrar el incidente), y un score de confianza. Si el agente ve la misma firma en otra entidad, recupera el diagnóstico anterior en vez de razonar desde cero — pero el mensaje de reuso es explícito en que es una coincidencia *estructural*, no una confirmación de que sea el mismo incidente real (la firma sin ancla de plataforma daba falsos positivos entre dominios no relacionados; con plataforma el problema se reduce pero no desaparece del todo — ver `AUDIT_REPORT.md`).

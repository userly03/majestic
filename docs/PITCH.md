...
> Verified across multiple internal testing rounds, the last one being live validation against a real DataHub instance.
...
- [x] **Full pipeline run end to end against a real DataHub instance** (2026-08-08): `doctor` -> `seed_demo_data.py` -> `diagnose --explain --write` -> `impact` -> memory reuse on a second entity. Found and fixed, along the way, a real UI bug (see `DATAHUB_UI_BUG_REPORT.md`) — exactly the kind of finding that only shows up running against something real, not against mocks.

**The method and class names used (`DataHubGraph.scroll_lineage`, `LineageDirection`, `DatasetPatchBuilder`, `StructuredPropertiesClass`, `StructuredProperties.from_yaml`) were confirmed by installing `acryl-datahub==1.7.0` in an isolated environment and inspecting the SDK directly**, not assumed from documentation — exactly the precaution this section called for, and one that in several concrete cases (the YAML field names, the retry timing, and the UI bug found during live validation) found real bugs the documentation wouldn't have revealed.
...
- [x] Public GitHub repo — verify the Apache 2.0 license shows in the "About" sidebar
- [x] Project description — this document + `README.md`
- [ ] Demo video <3 min, public YouTube/Vimeo — recorded, still needs to be uploaded publicly
- [ ] `examples/` with real outputs — currently generated by `scripts/generate_example_outputs.py` against `FakeDataHub` (real production code, fake graph); still needs a final regeneration against a live DataHub instance before submission. See `examples/README.md` for the detail and the exact commands to run.
